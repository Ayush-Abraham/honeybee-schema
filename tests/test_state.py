...
def test_aperture_state_abridged_bsdf():
    ...
    with open(file_path) as file:
        json_data = file.read()
    RadianceSubFaceStateAbridged.model_validate_json(json_data)


def test_aperture_state_abridged_electrochromic():
    file_path = os.path.join(
        target_folder, "aperture_state_abridged_electrochromic.json"
    )
    # RadianceSubFaceStateAbridged.parse_file(file_path)
    with open(file_path) as file:
        json_data = file.read()
...
def test_shade_state_abridged_snow():
    ...
    with open(file_path) as file:
        json_data = file.read()
    RadianceShadeStateAbridged.model_validate_json(json_data)

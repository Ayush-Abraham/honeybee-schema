...
class Point3D(NoExtraBaseModel):
    ...
    x: float = Field(..., description="Number for X coordinate.")

    y: float = Field(..., description="Number for Y coordinate.")

    z: float = Field(..., description="Number for Z coordinate.")
...
class Color(NoExtraBaseModel):
    ...
    r: int = Field(..., ge=0, le=255, description="Value for red channel.")

    g: int = Field(..., ge=0, le=255, description="Value for green channel.")

    b: int = Field(..., ge=0, le=255, description="Value for blue channel.")
...
class Mesh3D(NoExtraBaseModel):
    ...
    faces: List[
        Annotated[List[Annotated[int, Field(ge=0)]], Field(min_length=3, max_length=4)]
    ] = Field(
        ...,
        min_length=1,

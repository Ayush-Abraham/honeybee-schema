...
from pydantic import model_validator, StringConstraints, Field
...
class PeopleAbridged(IDdEnergyBaseModel):
    type: Annotated[str, StringConstraints(pattern="^PeopleAbridged$")] = (
        "PeopleAbridged"
    )

    people_per_area: float = Field(
        ..., ge=0, description="People per floor area expressed as [people/m2]"
    )
    ...
    activity_schedule: str | None = Field(
        default=None,
        min_length=1,
        max_length=100,
    ...
    def check_sum_fractions(self):
        "Ensure sum is less than 1."
        rad = self.radiant_fraction
        latent = self.latent_fraction
...
class People(PeopleAbridged):
    type: Annotated[str, StringConstraints(pattern="^People$")] = "People"
...
class LightingAbridged(IDdEnergyBaseModel):
    type: Annotated[str, StringConstraints(pattern="^LightingAbridged$")] = (
        "LightingAbridged"
    )

    watts_per_area: float = Field(
        ..., ge=0, description="Lighting per floor area as [W/m2]."
    )
    ...
    def check_sum_fractions(self):
        "Ensure sum is less than 1."
        return_air = self.return_air_fraction
        vis = self.visible_fraction
        rad = self.radiant_fraction
...
class Lighting(LightingAbridged):
    type: Annotated[str, StringConstraints(pattern="^Lighting$")] = "Lighting"
...
class _EquipmentBase(IDdEnergyBaseModel):
    watts_per_area: float = Field(
        ..., ge=0, description="Equipment level per floor area as [W/m2]."
    )
...
class ElectricEquipmentAbridged(_EquipmentBase):
    type: Annotated[str, StringConstraints(pattern="^ElectricEquipmentAbridged$")] = (
        "ElectricEquipmentAbridged"
    )


class ElectricEquipment(ElectricEquipmentAbridged):
    type: Annotated[str, StringConstraints(pattern="^ElectricEquipment$")] = (
        "ElectricEquipment"
    )
...
class GasEquipmentAbridged(_EquipmentBase):
    type: Annotated[str, StringConstraints(pattern="^GasEquipmentAbridged$")] = (
        "GasEquipmentAbridged"
    )


class GasEquipment(GasEquipmentAbridged):
    type: Annotated[str, StringConstraints(pattern="^GasEquipment$")] = "GasEquipment"
...
class ServiceHotWaterAbridged(IDdEnergyBaseModel):
    type: Annotated[str, StringConstraints(pattern="^ServiceHotWaterAbridged$")] = (
    ...
    def check_sum_fractions(self):
        "Ensure sum is less than 1."
        sens = self.sensible_fraction
        lat = self.latent_fraction
...
class ServiceHotWater(ServiceHotWaterAbridged):
    type: Annotated[str, StringConstraints(pattern="^ServiceHotWater$")] = (
        "ServiceHotWater"
    )
...
class InfiltrationAbridged(IDdEnergyBaseModel):
    type: Annotated[str, StringConstraints(pattern="^InfiltrationAbridged$")] = (
    ...
    constant_coefficient: float = Field(1, ge=0)

    temperature_coefficient: float = Field(0, ge=0)

    velocity_coefficient: float = Field(0, ge=0)


class Infiltration(InfiltrationAbridged):
    type: Annotated[str, StringConstraints(pattern="^Infiltration$")] = "Infiltration"
...
class VentilationAbridged(IDdEnergyBaseModel):
    type: Annotated[str, StringConstraints(pattern="^VentilationAbridged$")] = (
        "VentilationAbridged"
    )
    ...
    flow_per_area: float = Field(
        0, ge=0, description="Intensity of ventilation in [m3/s per m2 of floor area]."
    )
    ...
    flow_per_zone: float = Field(
        0, ge=0, description="Intensity of ventilation in m3/s for the entire Room."
    )
...
class Ventilation(VentilationAbridged):
    type: Annotated[str, StringConstraints(pattern="^Ventilation$")] = "Ventilation"
...
class SetpointAbridged(IDdEnergyBaseModel):
    ...
    type: Annotated[str, StringConstraints(pattern="^SetpointAbridged$")] = (
        "SetpointAbridged"
    )
    ...
    humidifying_schedule: str | None = Field(
        default=None,
        min_length=1,
        max_length=100,
    ...
    dehumidifying_schedule: str | None = Field(
        default=None,
        min_length=1,
        max_length=100,
...
class Setpoint(SetpointAbridged):
    ...
    type: Annotated[str, StringConstraints(pattern="^Setpoint$")] = "Setpoint"
    ...
    dehumidifying_schedule: Union[ScheduleRuleset, ScheduleFixedInterval] | None = (
        Field(
            default=None,
...
class ProcessAbridged(IDdEnergyBaseModel):
    type: Annotated[str, StringConstraints(pattern="^ProcessAbridged$")] = (
        "ProcessAbridged"
    )

    watts: float = Field(
        ..., ge=0, description="A number for the process load power in Watts."
    )

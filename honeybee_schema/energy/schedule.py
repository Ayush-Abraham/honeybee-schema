...
class ScheduleTypeLimit(EnergyBaseModel):
    ...
    type: Annotated[str, StringConstraints(pattern="^ScheduleTypeLimit$")] = (
        "ScheduleTypeLimit"
    )

    lower_limit: Union[NoLimit, float] = Field(
        default=NoLimit(), description="Lower limit for the schedule type or NoLimit."
    )

    upper_limit: Union[NoLimit, float] = Field(
        default=NoLimit(), description="Upper limit for the schedule type or NoLimit."
    )
...
class ScheduleDay(EnergyBaseModel):
    ...
    type: Annotated[str, StringConstraints(pattern="^ScheduleDay$")] = "ScheduleDay"
    ...
    # times: List[conlist(int, min_items=2, max_items=2)]
    times: list[
        Annotated[list[int], Annotated[List[int], Field(min_length=2, max_length=2)]]
    ] = Field(
        [[0, 0]],
    ...
    def check_times_values_match(self):
        "Ensure the length of the times matches the length of the values."
        times, vals = self.times, self.values
        assert len(times) == len(vals), (
            "Length of schedule times must match the schedule values. {} != {}.".format(
                len(times), len(vals)
            )
        )
        return self
...
class ScheduleRuleAbridged(DatedBaseModel):
    ...
    type: Annotated[str, StringConstraints(pattern="^ScheduleRuleAbridged$")] = (
    ...
    apply_sunday: bool = Field(
        False, description="Boolean noting whether to apply schedule_day on Sundays."
    )

    apply_monday: bool = Field(
        False, description="Boolean noting whether to apply schedule_day on Mondays."
    )

    apply_tuesday: bool = Field(
        False, description="Boolean noting whether to apply schedule_day on Tuesdays."
    )

    apply_wednesday: bool = Field(
        False, description="Boolean noting whether to apply schedule_day on Wednesdays."
    )

    apply_thursday: bool = Field(
        False, description="Boolean noting whether to apply schedule_day on Thursdays."
    )

    apply_friday: bool = Field(
        False, description="Boolean noting whether to apply schedule_day on Fridays."
    )

    apply_saturday: bool = Field(
        False, description="Boolean noting whether to apply schedule_day on Saturdays."
    )
...
class ScheduleRulesetAbridged(IDdEnergyBaseModel):
    ...
    type: Annotated[str, StringConstraints(pattern="^ScheduleRulesetAbridged$")] = (
    ...
    schedule_rules: List[ScheduleRuleAbridged] | None = Field(
        default=None,
    ...
    holiday_schedule: str | None = Field(
        default=None,
        min_length=1,
        max_length=100,
    ...
    summer_designday_schedule: str | None = Field(
        default=None,
        min_length=1,
        max_length=100,
        description="An identifier for the ScheduleDay that will be used for "
    ...
    winter_designday_schedule: str | None = Field(
        default=None,
        min_length=1,
        max_length=100,
    ...
    schedule_type_limit: str | None = Field(
        default=None,
        min_length=1,
        max_length=100,
...
class ScheduleRuleset(ScheduleRulesetAbridged):
    ...
    type: Annotated[str, StringConstraints(pattern="^ScheduleRuleset$")] = (
        "ScheduleRuleset"
    )

    schedule_type_limit: ScheduleTypeLimit | None = Field(
        default=None,
...
class ScheduleFixedIntervalAbridged(IDdEnergyBaseModel):
    ...
    # )
    type: Annotated[
        str, StringConstraints(pattern="^ScheduleFixedIntervalAbridged$")
    ] = "ScheduleFixedIntervalAbridged"
    ...
    def check_number_of_values(self):
        ...
        max_l = (
            timestep * 8760
            if len(start_date) == 3 and start_date[2]
            else timestep * 8784
        )
        if len(vals) > max_l:
            raise ValueError(
                "Number of schedule values must not exceed a full year, "
...
class ScheduleFixedInterval(ScheduleFixedIntervalAbridged):
    ...
    type: Annotated[str, StringConstraints(pattern="^ScheduleFixedInterval$")] = (
